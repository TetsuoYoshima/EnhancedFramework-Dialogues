...
namespace EnhancedFramework.Dialogues.Editor {
    /// <summary>
    /// Editor window used to edit <see cref="Dialogues.Dialogue"/> assets.
    /// <para/>
    ...
    public sealed class DialogueEditorWindow : EditorWindow {
        ...
        #region Window GUI
        /// <summary>
        /// Returns the first <see cref="DialogueEditorWindow"/> currently on screen.
        /// <br/> Creates and shows a new instance if there is none.
        /// </summary>
        /// <returns><see cref="DialogueEditorWindow"/> instance on screen.</returns>
        [MenuItem(FrameworkUtility.MenuItemPath + "Dialogue Editor", false, 10)]
        public static DialogueEditorWindow GetWindow() {
            DialogueEditorWindow _window = GetWindow<DialogueEditorWindow>("Dialogue Editor");
            _window.Show();
        ...
        private const string UndoRecordTitle = "Dialogue Change";
        private const string PreferencesKey = "DialoguePreferences";
        private const string NoDialogueMessage = "Open a dialogue asset to start editing its convent.";
        ...
        [SerializeReference] private static DialogueNode selectedNode = null;
        [SerializeReference] private static List<DialogueNode> nodes = new List<DialogueNode>();

        private readonly GUIContent openGUI = new GUIContent(" Open", "Select a new dialogue asset to edit.");
        private readonly GUIContent newGUI = new GUIContent(" New", "Creates a new dialogue asset and open it.");
        ...
        [SerializeField] private string dialoguePath = string.Empty;
        ...
        private void OnEnable() {
            ...
            if (!string.IsNullOrEmpty(_json)) {
                ...
                if (dialogue == null) {
                    dialogue = AssetDatabase.LoadAssetAtPath<Dialogue>(dialoguePath);
                }
            }
            ...
            removeSpeakerColorGUI.image = EditorGUIUtility.IconContent("CrossIcon").image;
        ...
        private void OnFocus() {
            ...
            if (_selection != dialogue) {

                if (_selection is Dialogue _dialogue) {

                    // Update selection.
                    SetDialogue(_dialogue, false);
                }
                else if (dialogue != null) {

                    // Select the editing dialogue while focused to display various parameters in the inspector.
        ...
        private void OnGUI() {
            ...
            using (var _scope = new GUILayout.ScrollViewScope(scroll)) {
                ...
                if (dialogue == null) {
                    GUILayout.Space(10f);
                    EditorGUILayout.HelpBox(NoDialogueMessage, UnityEditor.MessageType.Info);
                    return;
                }

                Undo.RecordObject(dialogue, UndoRecordTitle);
        ...
        private const string OpenFileTitle = "Open Dialogue";
        private const string FileExtension = "asset";

        private const string NewFileTitle = "Create New Dialogue";
        private const string NewFileDefaultName = Dialogue.FilePrefix + "NewDialogue";
        private const string NewFileMessage = "Please select a valid path to create a new dialogue";
        ...
        private void DrawToolbar() {
            using (var _scope = new GUILayout.HorizontalScope(EditorStyles.toolbar)) {
                ...
                if (GUILayout.Button(openGUI, EditorStyles.toolbarButton, GUILayout.Width(OpenButtonWidth))) {
                    ...
                    if (!string.IsNullOrEmpty(_file)) {
                        Dialogue _asset = AssetDatabase.LoadAssetAtPath<Dialogue>(_file.Remove(0, EnhancedEditorUtility.GetProjectPath().Length));

                        if (_asset != null) {
                            SetDialogue(_asset, true);
                        }
                    }
                }
                ...
                if (GUILayout.Button(newGUI, EditorStyles.toolbarButton, GUILayout.Width(NewButtonWidth))) {
                    ...
                    if (!string.IsNullOrEmpty(_file)) {
                        Dialogue _newDialogue = CreateInstance<Dialogue>();

                        AssetDatabase.CreateAsset(_newDialogue, _file);
                        AssetDatabase.Refresh();

                        Preset[] _presets = Preset.GetDefaultPresetsForObject(_newDialogue);
                        if (_presets.SafeFirst(out Preset _preset)) {
                            _preset.ApplyTo(_newDialogue);
                        }

                        SetDialogue(_newDialogue, true);
                    }
                }
            ...
            string GetFolderPath() {
                ...
                string _path = Path.GetDirectoryName(dialoguePath);
        ...
        private void DrawGUI() {
            ...
            using (var _global = new EditorGUILayout.HorizontalScope(EditorStyles.inspectorFullWidthMargins)) {
                ...
                if ((_event.type == EventType.DragUpdated) && (_event.mousePosition.y > _global.rect.y)) {
                    DragAndDrop.visualMode = ((DragAndDrop.objectReferences.Length == 1) && (DragAndDrop.objectReferences[0] == dialogue))
                ...
                using (var _vertical = new EditorGUILayout.VerticalScope(EditorStyles.inspectorFullWidthMargins, GUILayout.Width(position.width - _configurationWidth - ConfigurationSectionSpacing))) {
                    int _index = 0;
                    DrawDialogueNode(dialogue.Root, Rect.zero, 0, ref _index, out Rect _position);
                    ...
                    if (_fadeScope.visible) {
                        ...
                        using (var _vertical = new EditorGUILayout.VerticalScope(EditorStyles.inspectorFullWidthMargins, GUILayout.Width(_width))) {
                            ...
                            if ((selectedNodeIndexes.Count == 1)) {
                                ...
                                GUILayout.Space(5f);
                                EditorGUILayout.SelectableLabel(GetSelectedNode().GetEditorSpeakerName(dialogue.Settings), EditorStyles.miniTextField);
                            }
        ...
        private bool DrawDialogueNode(DialogueNode _node, Rect _neighbour, int _indent, ref int _index, out Rect _nodePosition) {
                ...
                using (var _scope = new EditorGUILayout.FadeGroupScope(_node.foldout.faded)) {
                    if (_scope.visible) {
                        ...
                        for (int i = 0; i < _node.nodes.Length; i++) {
                            if (!DrawDialogueNode(_node.nodes[i], _nodePosition, _indent + 1, ref _index, out _nodePosition)) {
                                return false;
                            }
                        }
                    }
            ...
            string _speaker = _node.GetEditorSpeakerName(dialogue.Settings);
            ...
            if ((_speakerIndex == -1) || dialogue.HasDuplicateName) {
        ...
        private bool OnDragAndDrop(Rect _position, int _index) {
            ...
            if ((_event.type == EventType.MouseDrag) && (_event.button == 0) && (dragTimer < EditorApplication.timeSinceStartup)) {
                ...
                DragAndDrop.objectReferences = new Object[] { dialogue };
                DragAndDrop.StartDrag("Dialogue Node(s) Drag");

                _event.Use();
            } else if (_event.type == EventType.DragPerform) {
                // Get destination parent node.
                DialogueNode _selected = nodes[_index];
                int _insert = 0;
                ...
                for (int i = 1; i < _index; i++) {
                    DialogueNode _node = nodes[i];

                    if (_node.isSelected) {
                        DialogueNode _parent = _selected.parent;
        ...
        private bool CanSelectNode(int _index) {
            DialogueNode _node = nodes[_index];
        ...
        private void CreateNode(Type _nodeType) {
            if (selectedNodeIndexes.Count == 0) {
                dialogue.AddNode(dialogue.Root, _nodeType).isSelected = true;
            } else {
                for (int i = 0; i < selectedNodeIndexes.Count; i++) {
                    DialogueNode _node = GetSelectedNodeAtIndex(i);
                    _node.isSelected = false;

                    dialogue.AddNode(_node, _nodeType).isSelected = true;
                }
            }
        ...
        private void DeleteSelectedNodes() {
            ...
            if (nodes[0].isSelected) {
                Array.Resize(ref dialogue.Root.nodes, 0);
            } else {
                selectedNodeIndexes.Sort();

                for (int i = selectedNodeIndexes.Count; i-- > 0;) {
                    dialogue.RemoveNode(GetSelectedNodeAtIndex(i));
                }
            }
        ...
        private void ShowContextMenu() {
            ...
            var _derived = TypeCache.GetTypesDerivedFrom(typeof(DialogueNode));
            ...
            if (selectedNodeIndexes.Count != 0) {
                bool _hasBuffer = DialogueNodeUtility.copyBuffer != null;
                ...
                if (selectedNodeIndexes.Count == 1) {
                    DialogueNode _selected = GetSelectedNodeAtIndex(0);
        ...
        private void CreateDefaultNode() {
            CreateNode(dialogue.DefaultNodeType);
        }
        ...
        private void CopyNode() {
            ...
            DialogueNodeUtility.copyBuffer = GetSelectedNodeAtIndex(0);
        }

        private void DuplicateNode() {
            DupplicateNode(dialogue.Root);
            RefreshNodes(false);

            // ----- Local Method ----- \\

            void DupplicateNode(DialogueNode _node) {
                for (int i = _node.nodes.Length; i-- > 0;) {
                    DialogueNode _innerNode = _node.nodes[i];

                    if (_innerNode.isSelected) {
                        DialogueNode _duplicate = Activator.CreateInstance(_innerNode.GetType()) as DialogueNode;
        ...
        private void PasteNodeValues() {
            DialogueNode _source = DialogueNodeUtility.copyBuffer;
            ...
            void ConvertNode(DialogueNode _node) {
                for (int i = _node.nodes.Length; i-- > 0;) {
                    DialogueNode _innerNode = _node.nodes[i];

                    if (_innerNode.isSelected) {
                        _innerNode = _innerNode.Transmute(dialogue, _source.GetType(), true, false);
        ...
        private void PasteNodeAsNew() {
            ...
            for (int i = selectedNodeIndexes.Count; i-- > 0;) {
                ...
                DialogueNode _new = Activator.CreateInstance(_source.GetType()) as DialogueNode;
        ...
        private void PasteNodeAsLink() {
            ...
            for (int i = selectedNodeIndexes.Count; i-- > 0;) {
                ...
                _link = Activator.CreateInstance(dialogue.DefaultLinkType) as DialogueLink;
        ...
        private void ChangeNodeType(object _type) {
            ...
            for (int i = 0; i < selectedNodeIndexes.Count; i++) {
                ...
                if (_index != -1) {
                    _node.Transmute(dialogue, _nodeType, true, false);
                }
            }
        ...
        internal static bool DrawInspector(Dialogue _dialogue) {
            if (!isActive || (_dialogue != Dialogue)) {
                return false;
            }

            // Simply draw the dialogue inspector on multi node selection or when the root node is selected.
            if ((selectedNodeIndexes.Count != 1) || (selectedNode == Dialogue.Root)) {
                return false;
            }
        ...
        /// </summary>
        public void SelectNode(DialogueNode _node, bool _unslectOthers = true) {
            for (int i = 0; i < nodes.Count; i++) {
                if (_node == nodes[i]) {
                    UnselectAllNodes();
                    SelectNode(i, true);
        ...
        /// </summary>
        public static DialogueNode GetSelectedNode() {
            return selectedNode;
        }
        #endregion

        #region Utility
        private static DialogueNode GetSelectedNodeAtIndex(int _index) {
            return nodes[selectedNodeIndexes[_index]];
        }

        /// <summary>
        /// Sets this window editing dialogue.
        /// </summary>
        public void SetDialogue(Dialogue _dialogue, bool _select = false) {

            if (_select && (_dialogue != dialogue)) {
                Selection.activeObject = _dialogue;
                EditorGUIUtility.PingObject(_dialogue);
            }

            dialogue = _dialogue;
            scroll = Vector2.zero;

            dialoguePath = AssetDatabase.GetAssetPath(_dialogue);
        ...
        private void RefreshDialogue(bool _unselect = true) {
            Dialogue = dialogue;
            serializedObject = (dialogue != null) ? new SerializedObject(dialogue) : null;
            ...
            DialogueNodeUtility.copyBuffer = null;

            if ((dialogue != null) && (dialogue.Root != null)) {
                RefreshNodes(_unselect);
                dialogue.isEditorCachedSpeakers = false;
            }
        }
        ...
        private void RefreshNodes(bool _unselect = true) {
            ...
            RegisterNode(dialogue.Root, null);
            RefreshSelectedNode();

            // ----- Local Method ----- \\

            void RegisterNode(DialogueNode _node, DialogueNode _parent) {
                nodes.Add(_node);
        ...
        private void RefreshLinks() {
            foreach (DialogueNode _node in nodes) {
                if ((_node is DialogueLink _link) && (_node.nodes.Length > 0)) {
        ...
        private void RefreshSelectedNode() {
            ...
            if (!RefreshSelectedNodeRecursive(dialogue.Root, _root)) {
                selectedProperty = null;
                selectedNode = dialogue.Root;
            }
            ...
            bool RefreshSelectedNodeRecursive(DialogueNode _node, SerializedProperty _property) {
                if (_node.isSelected) {
